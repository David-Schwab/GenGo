...
In Go werden Interfaces verwendet, um eine Menge von Methoden zu definieren, die ein Typ implementieren muss. Betrachten wir das folgende Interface Show:
...
Die Strukturen ermöglichen also die Wiederverwendbarkeit, wie im Fall der showTwice1 und showTwice2 Funktionen. Mit showTwice1 können wir verschiedene Typen kombinieren, die das Show Interface implementieren, ohne dass die Funktion für jeden spezifischen Typ neu geschrieben werden muss.

Außerdem wird von der Funktion showTwice2 Typensicherheit garantiert. Es wird sichergestellt, dass beide Parameter denselben Typ haben, was vom Compiler zur Kompilierungszeit überprüft wird.
...
Ein häufiges Problem tritt auf, wenn versucht wird, Type Assertions auf generische Typen anzuwenden. Betrachten wir die Funktion assert2:
...
Im folgenden soll kurz beschrieben und aufgezeigt werden, wie bei der Verwendung des Tools und dessen Ergenis vorgegangen wurden.

Als erstes muss das Tool aus der aggregation directory im usage interactive gestartet werden.

1. C:\....\GoAnalyse\goAnalyze\aggregation> go run aggregation.go -usage interactive

 Interactive mode...
Please enter commands:
(q or quit will exit this mode and end the programm)

Zunächst kann man über die Terminal Eingabe listProjects sehen, welche Go Projekte aus Github bisher geladen worden sind.
2. > listProjects
Projects: -
Da dies bisher nicht geschehen ist, ist die Liste der Projekte leer.

Durch die Eingabe getNewProjects 10, werden die 10 beliebtesten bzw. größten Go Projekte aus GitHub geladen.
3.> getNewProjects 10

request: https://api.github.com/search/repositories?q=language:go&per_page=10&page=0

Gibt man danach erneut in die Kommandozeile listProjects ein, können wir diese 10 Projekte sehen.
4. > listProjects      
Projects:
...
Entscheiden wir uns nun für eines der 10 Projekte, können wir dieses mit loadProject herunterladen.
...
Download done!
gin-gonic/gin is now loaded and ready to be processesed.

Mit processProject beginnen wir dann schließlich damit, dass gewählte Projekt (was in diesem Fall gin-gonic ist) nach generics zu analysieren.
...
Um uns die Ergbenisse der Analyse übersichtlich anzeigen zu lassen geben wir nach Ablauf der Analyse evalResult in das Terminal ein.
...
>
Aus der Anaylse erhalten wir also folgende Ergebnisse:
Wir haben 43 used generics. Davon sind keie most certain hits und alle bzw. 43 uncertain hits.
Hierbei sind most certain hits, diejenigen die ganz sicher generics sind und uncertain hits, diejenigen, die aussehen wie generics aber in Wirklickeit keine sind. Bzw.  man geht davon aus das es keine hits sind, also beispielsweise map aufrufe, array aufrufe, aufrufe von datentypen aus anderen bibliotheken die man nicht nachvollziehen kann.
Desweiteren erhält man als Resultat das die Typeinschrämkung "any" kein einziges mal verwendet wurde, genauso wie Typesets. Dafür wurden aber ingesamt 64 Type Assertions verwendet.
Alle Verwendungen von Generics oder auch Type assertions werden anhand des Files und der jeweiligen Zeile belegt.
...
Es wurden 185 Generics verwendet, jedoch alle als unsicher (uncertain hits) eingestuft.
...
any: 3
string: 2
Uncertain Hits: 182 Treffer, darunter:

uint64: 6
(Weitere Angaben folgen in der detaillierten Dokumentation)
...
Fehlerbehandlung und Standardwerte: Ein Nutzer nutzt Generics häufig für zwei Funktionen:

must[T any](t T, err error) T: Gibt den Wert zurück oder löst bei einem Fehler eine Panik aus.
...
Ein Beispiel für die sinnvolle Nutzung von Generics ist ein Pull Request in Prometheus, der zeigt, wie Generics für eine effizientere Implementierung von Sortieralgorithmen verwendet werden können.
...
Einige Bibliotheken und Projekte haben von der Einführung von Generics profitiert. Beispiel: das Paket golang.org/x/exp/maps, das nützliche Funktionen für Map-Datenstrukturen bereitstellt.
...
Eingeschränkte Nutzung: Einige Entwickler berichten, dass sie Generics selten brauchen oder verwenden, da sie in ihren Anwendungsfällen nicht notwendig sind.
...
Allgemeine Einschätzungen:
Wichtig für die Bibliotheken, weniger für den Anwendungs-Code: Viele Entwickler stimmen darin überein, dass Generics besonders nützlich sind, wenn sie in Bibliotheken verwendet werden, die dann von anderen genutzt werden können.
Verbesserung, aber kein Allheilmittel: Ein Entwickler beschreibt Generics als eine Verbesserung, jedoch keine bahnbrechende Veränderung

Die Diskussion auf Hacker News über die Einführung von Generics in Go ist facettenreich und beleuchtet verschiedene Perspektiven sowohl von Befürwortern als auch von Kritikern.

Argumente gegen die Verwendung von Generics in Go
Kritische Meinungen und Herausforderungen
Eingeschränkte Nutzung:

Einige Entwickler berichten, dass sie Generics selten brauchen oder verwenden, da sie in ihren Anwendungsfällen nicht notwendig sind.
Schwierigkeiten bei der Umsetzung:

Einige Nutzer haben Schwierigkeiten, Generics in Go zu verstehen und korrekt zu implementieren, insbesondere im Vergleich zu anderen Sprachen wie Java.
Refactoring-Kosten:

Viele Open-Source-Projekte übernehmen Generics nicht, weil die Zeit und Ressourcen für das Refactoring begrenzt sind.
Leistungsbedenken:

Es gibt Bedenken, dass die Einführung von Generics die Kompilierungszeiten verlangsamt hat. Auch wenn Go 1.20 diese Zeiten wieder verbessert hat, bleibt die Frage, ob die initiale Einführung notwendig war.
...
Geringe praktische Nutzung:

Einige Entwickler argumentieren, dass Generics in der Praxis nur selten verwendet werden und hauptsächlich in speziellen, weniger verbreiteten Bibliotheken zu finden sind.


Fazit zur Verwendung von Generics in Go
Die Einführung von Generics in Go war ein bedeutender Schritt zur Erweiterung der Sprachfunktionalität.

Die Einführung von Generics in Go bietet zweifellos Vorteile, insbesondere für die Erstellung flexibler und typsicherer Bibliotheken. Die Analyse mehrerer Projekte und Diskussionen in der Entwicklergemeinschaft zeigt jedoch, dass Generics trotz ihres Potenzials bislang nur begrenzt eingesetzt werden. Viele Entwickler finden Generics in ihren spezifischen Anwendungsfällen nicht notwendig oder haben Schwierigkeiten bei der Umsetzung. Die zusätzlichen Komplexitäten und möglichen Performanceeinbußen tragen ebenfalls dazu bei, dass Generics bisher nicht weit verbreitet sind.
